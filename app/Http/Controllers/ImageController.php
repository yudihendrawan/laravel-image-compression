...
class ImageController extends Controller
{
    ...
    public function compressImage(Request $request)
    {
        ...
        $img = $this->createFrequencyMatrix($imagePath);
    ...
    public function createFrequencyMatrix($imagePath)
    {
        ...
        dd($pixelFrequency);
        // Buat matriks frekuensi berdasarkan ukuran gambar

        $imageManager = new ImageManager(new Driver());
        $width = $imageManager->read($imagePath)->width();
        $height = $imageManager->read($imagePath)->height();

        $frequencyMatrix = [];

        for ($x = 0; $x < $width; $x++) {
            for ($y = 0; $y < $height; $y++) {
                $pixel = $imageManager->read($imagePath)->pickColor($x, $y);
                // $character = implode('', $pixel);
                $character = $pixel->toString(); // Convert array RGB menjadi string
                // Set frekuensi karakter piksel pada matriks
    ...
    {
        // Membaca data gambar sebagai piksel
        $imageData = file_get_contents($imagePath);

        // Hitung frekuensi karakter untuk setiap piksel
        $pixelFrequency = [];
